...
function sendMsgAsync(serviceName, methodName, bodyBytes, timeout = 10000) {
    return new Promise((resolve, reject) => {
        ...
        // 添加调试日志
        if (serviceName === 'gamepb.friendpb.FriendService' && (methodName === 'GetAll' || methodName === 'SyncAll')) {
            console.log('[DEBUG] sendMsgAsync 调用参数:');
            console.log('  serviceName:', serviceName);
            console.log('  methodName:', methodName);
            console.log('  bodyBytes 长度:', bodyBytes ? bodyBytes.length : 0);
            console.log('  bodyBytes (hex):', bodyBytes ? Buffer.from(bodyBytes).toString('hex') : 'null');
            console.log('  clientSeq:', seq);
            console.log('  serverSeq:', serverSeq);
        }
        ...
        const sent = sendMsg(serviceName, methodName, bodyBytes, (err, body, meta) => {
            ...
            if (err) {
                // 添加错误日志
                if (serviceName === 'gamepb.friendpb.FriendService' && (methodName === 'GetAll' || methodName === 'SyncAll')) {
                    console.log('[DEBUG] 好友请求返回错误:', err);
                    console.log('[DEBUG] 错误详情:', JSON.stringify(err, null, 2));
                }
                reject(err);
            } else {
                if (serviceName === 'gamepb.friendpb.FriendService' && (methodName === 'GetAll' || methodName === 'SyncAll')) {
                    console.log('[DEBUG] 好友请求成功');
                    console.log('[DEBUG] 返回 meta:', meta);
                }
                resolve({ body, meta });
            }
        });
    ...
    try {
        ...
        if (msgType === 2) {
            ...
            if (cb) {
                ...
                if (errorCode !== 0) {
                    // 添加详细的错误日志
                    if (meta.service_name === 'gamepb.friendpb.FriendService' && (meta.method_name === 'GetAll' || meta.method_name === 'SyncAll')) {
                        console.log('[DEBUG] 好友响应错误详情:');
                        ...
                        console.log('  client_seq:', clientSeqVal);
                        console.log('  server_seq:', toNum(meta.server_seq));
                        console.log('  完整 meta:', JSON.stringify(meta, null, 2));
                    }
